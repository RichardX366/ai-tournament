"""
rat.py — JAX-accelerated HMM rat tracker

HOW TO USE
----------
In agent.py __init__:
    from .rat import RatBelief
    self.rat = RatBelief(transition_matrix)

In agent.py play(), call update() FIRST, then query:
    noise, dist = sensor_data
    self.rat.update(board, noise, dist)

    belief              = self.rat.belief               # jnp array shape (64,), sums to 1.0
    best_xy             = self.rat.best_guess()         # (x, y) most likely rat cell
    best_cell, best_ev  = self.rat.best_search_target() # best cell + its EV
    ev                  = self.rat.ev_search((3, 4))    # EV of searching a specific cell
    go                  = self.rat.should_search()      # True if best EV > 0
